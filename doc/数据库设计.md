...
记录用于 MCP 端点鉴权的访问令牌。鉴权规则见 [MCPToken.md](MCPToken.md)。

| 字段名 | 数据类型 | 约束 | 默认值 | 描述 |
|-------|---------|------|-------|------|
| id | INTEGER | PRIMARY KEY | 自增 | 令牌ID |
| name | VARCHAR(255) | NOT NULL | | 令牌名称 |
| description | TEXT | | | 令牌描述 |
| token | VARCHAR(255) | NOT NULL | | 令牌密钥（明文，作为 Bearer） |
| created_at | INTEGER | | | 创建时间（UnixMS） |
| updated_at | INTEGER | | | 更新时间（UnixMS） |
| created_by | VARCHAR(255) | | | 创建人名 |
| updated_by | VARCHAR(255) | | | 更新人名 |
...
### 3.15 令牌工具关联表 (tb_token_tool)

记录令牌与工具的多对多关系，决定令牌可访问的工具范围。

| 字段名 | 数据类型 | 约束 | 默认值 | 描述 |
|-------|---------|------|-------|------|
| id | INTEGER | PRIMARY KEY | 自增 | 关联ID |
| token_id | INTEGER | NOT NULL | | 令牌ID，关联tb_token.id |
| tool_id | INTEGER | NOT NULL | | 工具ID，关联tb_tool.id |
| created_at | INTEGER | | | 创建时间（UnixMS） |
| created_by | VARCHAR(255) | | | 创建人名 |
...
tb_tool <-- tb_token_tool (tool_id)

tb_token <-- tb_token_tool (token_id)
