...
> MCP 传输端点（`/sse`、`/sse-{tag}`、`/messages/{path}`、`/mcp`、`/mcp-{tag}`）不在 `/api/v1` 前缀下，使用**独立的 MCP 访问令牌**鉴权（非 JWT）。详见 [MCPToken.md](MCPToken.md)。
...
管理 MCP 端点鉴权所用的访问令牌，需管理员 JWT。鉴权规则与客户端接入见 [MCPToken.md](MCPToken.md)。

### 获取令牌列表

- **URL**: `/token`
- **方法**: GET
- **描述**: 分页查询令牌，支持 `page`、`size`、`search`
- **响应**: 分页响应格式，每项含 `id`、`name`、`description`、`token`、`tool_ids`、审计字段

### 创建令牌

- **URL**: `/token`
- **方法**: POST
- **描述**: 创建令牌，令牌值由服务端生成（形如 `emcp_...`）并在响应中返回
- **请求体**:
...
- **响应**: 通用响应格式，`data` 为令牌详情（含明文 `token`）
...
- **URL**: `/token/{token_id}`
- **方法**: PUT
- **描述**: 更新名称/描述/绑定工具；`tool_ids` 传入则**整体替换**原有绑定
- **请求体**（字段均可选）:
...
- **URL**: `/token/{token_id}`
- **方法**: DELETE
- **描述**: 删除令牌及其工具关联；使用该令牌的 MCP 客户端立即失效
- **响应**: 通用响应格式